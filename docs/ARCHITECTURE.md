...
BLEND ON은 전자담배 관련 디바이스, 소모품, 니코틴 프리 플레이버, 대체 니코틴 상품을 다루지만 외형은 밝은 라이프스타일 편집샵으로 유지한다. 핵심 아키텍처 원칙은 가입 시 1회 성인 확인, 회원 등급, 상품 노출 제한, 서버 기준 주문 계산이다.

## 레이어

- UI: Nuxt pages, layouts, reusable components
- State: Pinia stores and Nuxt composables
- Client adapters: Firebase client plugin
