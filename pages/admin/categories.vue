<template>
  <main class="admin-page">
    ...
    <AdminFormSection title="카테고리 추가">
      ...
      <div class="form-row"><label>상위 카테고리</label><Select v-model="parentId"><option value="">없음</option><option v-for="category in productStore.categories" :key="category.id" :value="category.id">{{ category.name }}</option></Select></div>
      <div class="form-row"><label>성인 전용</label><Select v-model="adultOnly"><option value="false">아니오</option><option value="true">예</option></Select></div>
...
const addCategory = async () => {
  ...
  const parent = productStore.categories.find((category) => category.id === parentId.value)
