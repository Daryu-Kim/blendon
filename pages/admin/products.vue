<template>
  <main class="admin-page">
    ...
    <AdminTable :rows="filteredProducts" :columns="columns" row-key="id">
      ...
      <template #memberPrice="{ row }">{{ formatCurrency(row.memberPrice) }}</template>
      <template #stock="{ row }"><span :class="{ danger: row.stock <= 10 }">{{ row.stock }}</span></template>
      <template #isVisible="{ row }">{{ row.isVisible ? '노출' : '숨김' }}</template>
      <template #minUserGradeToView="{ row }">
        <span>{{ row.isAdultOnly ? '성인' : '일반' }} / {{ row.minUserGradeToView }}+</span>
      </template>
      <template #actions="{ row }">
        <div class="row-actions">
          <Button size="sm" variant="ghost" :to="`/admin/products/${row.id}`">수정</Button>
          <Button size="sm" variant="ghost" @click="productStore.toggleVisibility(row.id)">노출전환</Button>
          <Button size="sm" variant="danger" @click="productStore.softDeleteProduct(row.id)">삭제</Button>
        </div>
      </template>
    </AdminTable>
  </main>
...
const filteredProducts = computed(() => {
  ...
  return productStore.products.filter((product) => {
    ...
    const matchesCategory = !categoryId.value || product.categoryIds.includes(categoryId.value)
