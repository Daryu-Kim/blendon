<template>
  <main class="admin-page">
    <div class="section-title">
      <div>
        <h1>주문 관리</h1>
        <p>주문 검색, 결제 상태, 배송 상태, 취소/환불 요청 처리를 위한 기본 화면입니다.</p>
      </div>
    </div>
    ...
    <AdminTable :rows="orders.orders" :columns="columns" row-key="id">
      ...
      <template #actions="{ row }">
        <div class="row-actions">
          <Select :model-value="row.orderStatus" @update:model-value="orders.setOrderStatus(row.id, $event as typeof row.orderStatus)">
          ...
          <Select :model-value="row.deliveryStatus" @update:model-value="orders.setDeliveryStatus(row.id, $event as typeof row.deliveryStatus)">
