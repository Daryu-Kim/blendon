<template>
  <div class="image-uploader">
    ...
    <input type="file" accept="image/*" :disabled="uploading" @change="uploadFile">
    <Input :model-value="modelValue" placeholder="이미지 URL 또는 Storage 경로" @update:model-value="$emit('update:modelValue', $event)" />
    <p>{{ helperText }}</p>
  </div>
</template>
...
const props = withDefaults(defineProps<{ modelValue: string; pathPrefix?: string }>(), {
  pathPrefix: 'uploads'
})
const emit = defineEmits<{ 'update:modelValue': [value: string] }>()

const uploading = ref(false)
const helperText = computed(() => (uploading.value ? '이미지를 업로드하는 중입니다.' : '파일 업로드 또는 URL 직접 입력을 사용할 수 있습니다.'))
