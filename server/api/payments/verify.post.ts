import { failOrderPayment, confirmPaidOrder } from '~/server/utils/order-service'
...
export default defineEventHandler(async (event) => {
  ...
  if (!body.orderId || !body.paymentId) {
    throw createError({ statusCode: 400, statusMessage: '주문 또는 결제 식별자가 누락되었습니다.' })
  }
  ...
  if (payment.status === 'PAID' && payment.paidAmount === order.totalAmount) {
    ...
    return { order: saveServerOrder(confirmPaidOrder(order, payment.paymentId)) }
  }
