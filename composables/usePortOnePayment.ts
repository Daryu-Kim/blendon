...
export const usePortOnePayment = () => {
  ...
  const requestPayment = async (order: Order) => {
    ...
    try {
      const paymentId =
        order.portonePaymentId || order.paymentId || `payment-${order.orderNo}`;
      ...
      if (!storeId || !channelKey)
        throw new Error("PortOne 공개 설정이 필요합니다.");
      ...
      const response = await PortOne.requestPayment({
        ...
        payMethod: ({
          ...
          kakaopay: "EASY_PAY",
          naverpay: "EASY_PAY",
        }[order.paymentMethod] || "CARD") as Parameters<
      ...
      error.value =
        e instanceof Error ? e.message : "결제창 호출에 실패했어요.";
      throw e;
    } finally {
      loading.value = false;
    }
