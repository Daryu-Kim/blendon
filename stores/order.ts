...
const writeOrders = (orders: Order[]) => {
  if (import.meta.client) localStorage.setItem(ordersKey, JSON.stringify(orders))
}
...
export const useOrderStore = defineStore('order', {
  ...
  getters: {
    myOrders: (state) => {
      ...
      return state.orders.filter((order) => !uid || order.userId === uid || order.userId === 'mock-user')
    },
    pendingPaymentCount: (state) => state.orders.filter((order) => order.paymentStatus === 'pending' || order.paymentStatus === 'ready').length,
    preparingDeliveryCount: (state) => state.orders.filter((order) => order.deliveryStatus === 'ready').length,
    todayOrderCount: (state) => state.orders.length,
    todayRevenue: (state) => state.orders.filter((order) => order.paymentStatus === 'paid').reduce((sum, order) => sum + order.totalAmount, 0),
    recentOrders: (state) => [...state.orders].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 8)
  },
  actions: {
    hydrate() {
    ...
    async createPendingOrder(input: CheckoutInput) {
      ...
      try {
        ...
        const token = firebase.enabled && firebase.auth?.currentUser ? await firebase.auth.currentUser.getIdToken() : ''
    ...
    async verifyPayment(orderId: string, paymentId: string) {
      ...
      const index = this.orders.findIndex((order) => order.id === result.order.id)
