...
import type { AgreementConsent, TermsAgreement, UserProfile } from '~/types/domain'
...
const consent = (accepted = false, agreedAt: string | null = null): AgreementConsent => ({ accepted, agreedAt })
...
const createDefaultProfile = (uid: string, email: string, displayName = '블렌드 고객', input?: Partial<SignUpProfileInput>): UserProfile => {
...
const normalizeConsent = (value: unknown, fallbackAt: string | null): AgreementConsent => {
  if (typeof value === 'boolean') return consent(value, value ? fallbackAt : null)
  if (value && typeof value === 'object' && 'accepted' in value) {
    const record = value as Partial<AgreementConsent>
    return consent(Boolean(record.accepted), record.agreedAt || (record.accepted ? fallbackAt : null))
  }
  return consent()
}

const normalizeTermsAgreement = (terms?: Partial<TermsAgreement> | Record<string, unknown>): TermsAgreement => {
  const fallbackAt = typeof terms?.agreedAt === 'string' ? terms.agreedAt : null
  return {
...
const writeStoredProfile = (profile: UserProfile | null) => {
  ...
  else sessionStorage.setItem(storageKey, JSON.stringify(toSessionState(profile)))
}
...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    async init() {
      ...
      if (!firebase.enabled || !firebase.auth || !firebase.db) {
        ...
        if (runtime.public.enableMockAuth && session) {
          const mockProfile = mockUsers.find((user) => user.uid === session.uid)
          this.profile = normalizeProfile(mockProfile || createDefaultProfile(session.uid, session.email, session.displayName, { loginId: session.loginId }))
        } else {
          this.profile = null
        }
      ...
      await new Promise<void>((resolve) => {
        onAuthStateChanged(firebase.auth!, async (user) => {
          ...
          if (snap.exists()) {
            ...
            const profile = createDefaultProfile(user.uid, user.email || '', user.displayName || '블렌드 고객')
      ...
      try {
        return await useGlobalLoading().withLoading(async () => {
          ...
          if (firebase.enabled && firebase.auth && firebase.db) {
            ...
            const snap = await getDoc(doc(firebase.db, 'users', credential.user.uid))
            this.profile = snap.exists()
              ? normalizeProfile(snap.data() as UserProfile)
              : createDefaultProfile(credential.user.uid, email, credential.user.displayName || '블렌드 고객')
            writeStoredProfile(this.profile)
            return this.profile
          }

          if (!useRuntimeConfig().public.enableMockAuth) throw new Error('Firebase Authentication 설정이 필요합니다.')
          const preset = mockUsers.find((user) => user.email === email)
          const profile = normalizeProfile(preset || createDefaultProfile(`mock-${Date.now()}`, email, email.split('@')[0] || '블렌드 고객'))
          this.profile = profile
          writeStoredProfile(profile)
          return profile
        }, '로그인 처리 중')
      } catch (error) {
        this.error = error instanceof Error ? error.message : '로그인에 실패했어요.'
        throw error
      } finally {
        this.loading = false
      }
    },
    async signUp(email: string, password: string, displayName: string, input?: Partial<SignUpProfileInput>) {
      this.loading = true
      this.error = ''
      try {
        return await useGlobalLoading().withLoading(async () => {
          ...
          if (firebase.enabled && firebase.auth && firebase.db) {
            ...
            const profile = createDefaultProfile(credential.user.uid, email, displayName, input)
          ...
          if (!useRuntimeConfig().public.enableMockAuth) throw new Error('Firebase Authentication 설정이 필요합니다.')
          const profile = createDefaultProfile(`mock-${Date.now()}`, email, displayName, input)
          this.profile = profile
          writeStoredProfile(profile)
          return profile
        }, '회원가입 처리 중')
      } catch (error) {
        this.error = error instanceof Error ? error.message : '회원가입에 실패했어요.'
        throw error
      } finally {
        this.loading = false
      }
    ...
    async signOut() {
      const firebase = useNuxtApp().$firebase
      if (firebase.enabled && firebase.auth) await firebaseSignOut(firebase.auth)
      this.profile = null
      writeStoredProfile(null)
    },
    async saveProfile(updates: Partial<UserProfile>) {
      if (!this.profile) return
      const updated = { ...this.profile, ...updates, updatedAt: new Date().toISOString() }
      this.profile = updated
      writeStoredProfile(updated)
      const firebase = useNuxtApp().$firebase
      if (firebase.enabled && firebase.db) {
        await updateDoc(doc(firebase.db, 'users', updated.uid), { ...updates, updatedAt: serverTimestamp() })
      }
    },
