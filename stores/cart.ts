...
const writeCart = (items: CartItem[]) => {
  if (import.meta.client) localStorage.setItem(cartStorageKey, JSON.stringify(items))
}
...
export const useCartStore = defineStore('cart', {
    ...
    detailedItems: (state) => {
      ...
      return state.items
        .map((item) => {
          ...
          const option = product?.options.find((candidate) => candidate.optionId === item.optionId)
          if (!product || !option) return null
          const unitPrice = currentUnitPrice(product, auth.profile) + option.additionalPrice
          return { ...item, product, option, unitPrice, totalPrice: unitPrice * item.quantity }
        })
        .filter(Boolean)
    },
    subtotal(): number {
      return this.detailedItems.reduce((sum, item) => sum + (item?.totalPrice || 0), 0)
    },
    deliveryFee(): number {
  ...
  actions: {
    ...
    add(productId: string, optionId: string, quantity = 1) {
      ...
      const existing = this.items.find((item) => item.productId === productId && item.optionId === optionId)
