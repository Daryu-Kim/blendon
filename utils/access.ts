...
export const hasAdminAccess = (user?: Pick<UserProfile, 'role'> | null) => Boolean(user && adminRoles.includes(user.role))

export const hasRoleAtLeast = (user: Pick<UserProfile, 'role'> | null | undefined, role: Role) =>
  Boolean(user && roleRank[user.role] >= roleRank[role])

export const gradeLevel = (grade: GradeCode) => gradePolicy[grade]?.level ?? 0
export const gradeDiscountRate = (grade: GradeCode) => gradePolicy[grade]?.discountRate ?? 0

export const hasGradeAtLeast = (userGrade: GradeCode | undefined, minGrade: GradeCode) =>
  gradeLevel(userGrade || 'BASIC') >= gradeLevel(minGrade)

export const canViewProduct = (product: Product, user?: UserProfile | null) => {
  if (!product.isVisible || product.status === 'deleted' || product.status === 'hidden') return false
  if (!user) return !product.isAdultOnly && product.minUserGradeToView === 'BASIC'
...
export const shouldHidePrice = (product: Product, user?: UserProfile | null) => {
  if (!user && product.isPriceHiddenBeforeLogin) return true
  if (product.isPriceHiddenBeforeAdultVerification && (!user || !user.isAdultVerified)) return true
  if (product.isAdultOnly && (!user || !user.isAdultVerified)) return true
  return false
}

export const currentUnitPrice = (product: Product, user?: UserProfile | null) => {
  ...
  const discounted = Math.round(memberBase * (1 - gradeDiscountRate(user.userGrade) / 100))
...
export const discountRate = (product: Product, user?: UserProfile | null) => {
  ...
  return Math.max(0, Math.round(((product.compareAtPrice - price) / product.compareAtPrice) * 100))
}

export const maskRestrictedText = (product: Product, user?: UserProfile | null) => {
